  ...
  methods: {
    init(size) {
      ...
      class Particle {
        ...
        accelerate() {
          this.acc.magnitude =
            this.acc.magnitude > this.maxA ? this.acc.setMagnitude(this.maxA) : this.acc.magnitude;
          this.v.add(this.acc);
        }
        ...
        update() {
          ...
          this.v.magnitude =
            this.v.magnitude > this.maxV ? this.v.setMagnitude(this.maxV) : this.v.magnitude;
          this.x += this.v.x;
          this.y += this.v.y;
        ...
        render(context, trailContext = null) {
          ...
          if (this.trail && trailContext) {
            ...
            trailContext.strokeStyle = settings.COLOR;
        ...
        gravitateStarCluster(cluster) {
          ...
          for (let i = 0; i < cluster.length; i++) {
            ...
            if (Particle.prototype.isPrototypeOf(star)) {
              let v = new Vector();
              const d = Math.sqrt(
                (this.x - star.x) * (this.x - star.x) + (this.y - star.y) * (this.y - star.y),
              );
              const attractiveForce = (star.mass * this.mass) / (d * d);
              v.setAngle(Math.atan2(star.y - this.y, star.x - this.x)).setMagnitude(
                attractiveForce,
              );
              gV = gV.add(v);
            } else {
              throw new Error(
                'The argument supplied to the gravitateStarCluster function must be an array of particles',
              );
            }
          }
      ...
      let cW = (particlesCanvas.width = trailCanvas.width = size[0]);
      let cH = (particlesCanvas.height = trailCanvas.height = size[1]);

      //animation constants
      const settings = {
        STAR_MASS: 20000, // 恒星质量
        PLANET_MASS: 20,
        PLANET_V_X: 2,
        P_TRAIL: true,
        P_MAX_VELOCITY: 10, // 最大速度
        P_MAX_ACC: 0.2, // 最大加速度
        P_MIN_VELOCITY: 5,
        PARTICLE_NUM: 175, // 粒子数量
        BOUNDS: false, // 粒子边界
        TRAIL_LENGTH: 200, // 拖尾
      ...
      for (let i = 0; i < settings.PARTICLE_NUM; i++) {
        const planet = new Planet({
          ...
          // tasteTheRainbow: settings.TASTETHERAINBOW,
          v: new Vector(
            Math.random() < 0.5 ? -settings.P_MIN_VELOCITY : settings.P_MIN_VELOCITY,
            0,
          ),
        });
      ...
      function animate() {
        ...
        paintCanvas('transparent', settings.TRAIL_CTXT);
      ...
      gui
        .add(settings, 'STAR_MASS', 5000, 20000)
        .name('恒星质量')
      ...
      gui.add(settings, 'BOUNDS').name('粒子边界');
      gui
